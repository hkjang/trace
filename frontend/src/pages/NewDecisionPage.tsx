...
export default function NewDecisionPage() {
  ...
  const [form, setForm] = useState({ title: '', decision: '', category: 'technology', teamId: '', reason: '', assumptions: '', evidenceTitle: '', evidenceContent: '', knownAt: toLocalDateTime(), expectation: '', successCriteria: '', invalidationConditions: '', confidence: 72, decidedAt: toLocalDateTime(), reviewAt: '', alternatives: [] as string[] })
  ...
  return <div className="thought-page">
    ...
    <main className="thought-stage">
      ...
      <div className="answer-block">
        ...
        {step === 2 && <><input className="thought-title" value={form.evidenceTitle} onChange={e => update('evidenceTitle', e.target.value)} placeholder="근거의 제목" autoFocus /><textarea className="thought-input" value={form.evidenceContent} onChange={e => update('evidenceContent', e.target.value)} placeholder="이 근거에서 중요했던 내용" rows={4} /><label className="known-at">YOU KNEW THIS <input type="datetime-local" value={form.knownAt} onChange={e => update('knownAt', e.target.value)} /></label></>}
