...
export default function DecisionPage() {
  ...
  const [evidenceForm,setEvidenceForm]=useState({title:'',content:'',stance:'neutral'})
  ...
  const runAI = async (mode: 'review' | 'counter' | 'replay') => {
  ...
  const addEvidence=async(event:FormEvent)=>{event.preventDefault();try{await api(`/api/v1/decisions/${id}/evidence`,{method:'POST',body:JSON.stringify({title:evidenceForm.title,type:'note',source:'',content:evidenceForm.content,snapshot:'',reliability:null,stance:evidenceForm.stance,publishedAt:null,knownAt:new Date().toISOString()})});setEvidenceForm({title:'',content:'',stance:'neutral'});setShowEvidence(false);await load()}catch(err){setError(err instanceof Error?err.message:'근거를 추가하지 못했습니다.')}}
  ...
  return <div className={isReplay ? 'decision-page replay-mode' : 'decision-page'}>
    <header className="decision-topbar"><Link to="/" className="text-link"><ArrowLeft size={17} />시간의 흐름</Link><div className="mode-indicator"><span className={isReplay ? 'replay-light active' : 'replay-light'} />{isReplay ? `REPLAY · ${formatDate(new Date(replayAt!).toISOString(), true)}` : 'LIVE TRACE'}</div><div className="top-actions">{config?.workflow.approvalRequired && ['draft', 'rejected'].includes(decision.workflowState) && <button className="button amber" disabled={approvalBusy} onClick={() => void submitApproval()}><Send size={16} />팀장 검토 요청</button>}<span className={`state-label ${decision.workflowState}`}>{decision.workflowState === 'not_required' ? decision.status : decision.workflowState}</span></div></header>
    ...
    <div className="decision-workspace">
      ...
      <aside className="decision-detail custom-scroll"><div className="detail-head"><span className="object-kind">{decision.category.toUpperCase()} DECISION</span><h1>{decision.title}</h1><p>{decision.decision}</p><div className="detail-date mono">{formatDate(decision.decidedAt, true).toUpperCase()}</div></div>
        <div className="detail-confidence"><div><span>당시 확신</span><strong>{decision.confidence}%</strong></div><div className="mini-bar"><i style={{ width: `${decision.confidence}%` }} /></div><small>{decision.confidence > 80 ? 'Highly confident' : decision.confidence > 60 ? 'Confident' : 'Leaning'}</small></div>
        ...
        <DetailSection title="당시 알고 있던 근거" count={decision.evidence?.length} icon={<Eye size={17} />}>{decision.evidence?.map(item => <article className={`evidence-item ${item.stance}`} key={item.id}><div><strong>{item.title}</strong><span>{item.stance}</span></div><p>{item.content}</p><small>YOU KNEW THIS · {formatDate(item.knownAt)}</small></article>)}{isReplay && hiddenCount > 0 && <div className="ghost-card"><Layers3 size={17} /><span>이후에 추가된 정보 {hiddenCount}개</span><small>내용은 Replay에서 숨겨집니다</small></div>}{!isReplay&&<>{showEvidence?<form className="inline-capture" onSubmit={addEvidence}><input value={evidenceForm.title} onChange={e=>setEvidenceForm({...evidenceForm,title:e.target.value})} placeholder="새 근거 제목" required/><textarea value={evidenceForm.content} onChange={e=>setEvidenceForm({...evidenceForm,content:e.target.value})} placeholder="무엇을 알게 되었나요?" rows={2}/><select value={evidenceForm.stance} onChange={e=>setEvidenceForm({...evidenceForm,stance:e.target.value})}><option value="support">지지</option><option value="neutral">중립</option><option value="against">반대</option></select><button className="button secondary">근거 추가</button></form>:<button className="text-link add-inline" onClick={()=>setShowEvidence(true)}><Plus size={15}/>지금 알게 된 근거 추가</button>}</>}</DetailSection>
        ...
        <section className="ai-layer"><div className="ai-head"><div><Sparkles size={18} /><span>TRACE INSIGHT</span></div><BrainCircuit size={20} /></div>{aiText ? <div className="ai-output">{aiText}</div> : <p>내 판단을 먼저 읽은 뒤, Trace의 보라색 해석 레이어를 열어보세요.</p>}<div className="ai-actions"><button className="button purple" disabled={aiBusy} onClick={() => void runAI('review')}>{aiBusy ? <Pause size={16} /> : <Play size={16} />}{isReplay ? '당시 정보만으로 평가' : '판단 품질 분석'}</button><button className="button ghost" disabled={aiBusy} onClick={() => void runAI('counter')}>가장 강한 반론</button></div></section>
      </aside>
    </div>
